            ...
            if exam.update():
                ...
                for k in range(1, len(delete_classId) - 1):
                    ...
                    for stuID in listStuid:
                        ...
                        and exam_ex_id = $exam_ex_id",vars={'student_st_id': stuID.student_st_id,'exam_ex_id':exam.ex_id })
...
class SelectAllExam:
    def POST(self):
        ...
        currentPage = int(params.currentPage) - 1
        lists = model.Exam_model.query('select * from exam order by ex_time_end desc limit %s,%s' % (currentPage * 10, 10))
    ...
    def POST(self):
        ...
        if params.ex_name == '':
            ...
            lists = model.Exam_model.query('select * from exam where ex_name like \
              ...
              ex_time_end desc limit %s,%s' % (params.ex_name, "'" + params.ex_time_start + "'", "'" + params.ex_time_end + "'", \
                                       currentPage * 10, 10))
            result = model.Exam_model.query('select count(*) from exam where ex_name like \
